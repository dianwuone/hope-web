...
更新时间：`2026-05-25 00:00:00 +08:00`

现在后台“内容管理 -> 同步与 AI 发布”页已经支持直接上传本地 SQLite 数据库文件。

这个功能的目标不是只补表补字段，而是把本地最新 `app.db` 直接同步到服务器，优先让服务器使用这份新数据库，从而同时保证：
...
1. 在后台选择本地 `.db` / `.sqlite` / `.sqlite3` 文件上传
2. 服务端先校验它是不是有效的 SQLite 数据库
3. 自动备份当前线上数据库到 `backend/data/backups/`
4. 用上传的新数据库替换当前运行库
5. 替换后再执行一次结构同步，补齐新增字段或缺失表

适用场景：

1. 本地数据库是当前权威数据源
2. 本地调试时不仅改了代码，也新增或修正了数据库内容
3. 希望上线后服务器直接使用本地这份完整数据库

注意事项：

1. 这是“整库替换 + 自动备份”方案，不是双向合并
2. 如果线上已经积累了只存在于线上、但本地没有的新数据，上传后会被本地库覆盖
3. 更适合开发期、内测期或内容以本地维护为主的阶段
4. 上传完成后，后台会返回当前数据库路径、备份路径、执行时间和补结构 SQL

保底方案：

1. 如果你只想补结构，不想替换数据，继续使用“数据库同步”
2. 如果上传后的结果不对，可以用 `backend/data/backups/` 下的时间戳备份库回滚
